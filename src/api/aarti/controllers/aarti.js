...
module.exports = createCoreController('api::aarti.aarti', ({ strapi }) => ({
    ...
    async find(ctx) {
        ...
        const [data, total] = await Promise.all([
            strapi.entityService.findMany('api::aarti.aarti', { populate, start, limit }),
    ...
    async findOne(ctx) {
        const { id } = ctx.params;
        const data = await strapi.entityService.findOne('api::aarti.aarti', id, { populate });
        if (!data) return ctx.notFound(`Aarti with id "${id}" not found`);
        ctx.body = data;
    },

    async findBySlug(ctx) {
        const { slug } = ctx.params;
        const data = await strapi.entityService.findMany('api::aarti.aarti', {
            filters: { Slug: slug }, populate,
        });
        if (!data[0]) return ctx.notFound(`Aarti with slug "${slug}" not found`);
        ctx.body = data[0];
    },
