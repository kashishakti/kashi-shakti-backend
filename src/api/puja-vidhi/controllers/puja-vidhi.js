...
module.exports = createCoreController('api::puja-vidhi.puja-vidhi', ({ strapi }) => ({
    ...
    async find(ctx) {
        ...
        const [data, total] = await Promise.all([
            strapi.entityService.findMany('api::puja-vidhi.puja-vidhi', {
                populate, sort: { createdAt: 'desc' }, start, limit, locale,
            }),
            strapi.entityService.count('api::puja-vidhi.puja-vidhi', { locale }),
        ]);
    ...
    async findOne(ctx) {
        const { id } = ctx.params;
        const { locale } = ctx.query;
        const data = await strapi.entityService.findOne('api::puja-vidhi.puja-vidhi', id, { populate, locale });
        if (!data) return ctx.notFound(`Puja Vidhi with id "${id}" not found`);
        ctx.body = data;
    },
    ...
    async findBySlug(ctx) {
        ...
        const { locale } = ctx.query;
        const data = await strapi.entityService.findMany('api::puja-vidhi.puja-vidhi', {
            filters: { Slug: slug }, populate, locale,
        });
        if (!data[0]) return ctx.notFound(`Puja Vidhi with slug "${slug}" not found`);
        ctx.body = data[0];
    },
